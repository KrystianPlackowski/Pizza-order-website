...
class ItemName(models.Model):
    parent = models.ForeignKey(ItemKind, on_delete=models.CASCADE, related_name="name_list", verbose_name='Kind')
    name = models.CharField(max_length=64, blank=False, default='')

    def __str__(self):
        return f"({self.parent.kind}) - {self.name}"

    def kind(self):
        return self.parent.kind

class ItemSize(models.Model):
    parent = models.ForeignKey(ItemKind, on_delete=models.CASCADE, related_name="size_list", verbose_name='Kind')
    size = models.CharField(max_length=64, blank=False, default='')

    def __str__(self):
        return f"({self.parent.kind}) - {self.size}"
...
class MenuItem(models.Model):
    ...
    def __str__(self):
        return f"{self.id} - {self.name_parent.parent.kind} - {self.name_parent.name} - {self.size_parent.size} - {self.price}$"

    def kind(self):
        return self.name_parent.parent.kind
